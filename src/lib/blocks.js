...
export default function (vm) {
    ...
    const jsonForMenuBlockFINDMODEL = function (name, menuOptionsFn, colors, start) {
        return {
            message0: '%1',
            args0: [
                {
            ...
            colourTertiary: colors.secondary,
            outputShape: ScratchBlocks.OUTPUT_SHAPE_ROUND
        };
    };
    const jsonForMenuBlockAnimation = function (name, menuOptionsFn, colors, start) {
        return {
            message0: '%1',
            args0: [
                {
                    ...
                    name: name,
                    options: function () {
                        return menuOptionsFn() ? menuOptionsFn() : start
                    }
                }
            ],
            inputsInline: true,
    ...
    const jsonForSensingMenus = function (menuOptionsFn) {
        return {
            ...
            colourSecondary: ScratchBlocks.Colours.sensing.tertiary,
    ...
    const spriteMenuAnimation = function () {
        let spritesData = [];
        let currentTarget =  vm.runtime.targets.find(item=>item.id==vm.editingTarget.id);
        if(currentTarget && currentTarget.animationArr){
        spritesData = currentTarget.animationArr.map(obj => [obj.animationName,String(obj.index)]);
        return spritesData  
        }
    ...
    ScratchBlocks.Blocks.motion_findmodel_menu.init = function () {
        const mouse = ScratchBlocks.ScratchMsgs.translate('SENSING_TOUCHINGOBJECT_POINTER', 'mouse-pointer');
        const json = jsonForMenuBlockFINDMODEL('TOWARDS', spriteMenu, motionColors, [
            [mouse, '_mouse_'],
        ]);
        this.jsonInit(json);
    };
    ScratchBlocks.Blocks.looks_animation_menu.init = function () {
        const mouse = ScratchBlocks.ScratchMsgs.translate('SENSING_TOUCHINGOBJECT_POINTER', 'mouse-pointer');
        let spriteMenuFn,json
        if(vm.editingTarget.animationArr && vm.editingTarget.animationArr.length ){
            spriteMenuFn = spriteMenuAnimation
            json = jsonForMenuBlockAnimation('TOWARDS', spriteMenuAnimation, looksColors, [[mouse, '_mouse_']]);
        }else{
            spriteMenuFn = spriteMenu
            json = jsonForMenuBlock('TOWARDS', spriteMenu, looksColors,[[mouse, '_mouse_']]);
        }
        this.jsonInit(json);
    };
